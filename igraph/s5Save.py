...
def saveResults(root, level, cluster):
	...
	for line in txt.split('\n'):
		toks = line.split('[]')
		if(len(toks) > 2):
			clus = int(toks[0]); score = int(toks[1]); name = toks[2];
			isComplete = '1';
			if score > 1.5 and clus != 0 and level<4:
				isComplete = '0'
			query = "INSERT INTO `wg_cluster` (`id`, `parent`, `name`, `level`, `score`, `complete`) VALUES (NULL, '"+str(cluster)+"', '"+name+"', '"+str(level+1)+"', '"+str(score)+"', '"+isComplete+"');"
			cur.execute(query)
