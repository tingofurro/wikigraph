...
def isBreak(child): # should we stop there
	if child.name is not None and child.name == 'div' and child.get('id') is not None and child.get('id') == 'toc':
		return True
	if child.name is not None and (child.name == 'h2' or child.name == 'table'):
		return True
	return False
def isValid(child):
	if child.name is not None and child.name == 'div' and child.get('id') is not None and child.get('id') == 'toc':
...
def getContent(artId):
	if not os.path.isfile('summary/'+str(artId)+'.txt'):
		f = open('../data/'+str(artId)+'.txt')
		htm = f.read()
		...
		f = open('summary/'+str(artId)+'.txt', 'w')
		f.write(content)
		f.close()

cur.execute("SELECT id FROM wg_page ORDER BY id")
