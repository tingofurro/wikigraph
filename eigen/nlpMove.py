...
def genName(inText, outText, vocabValue, vocabIndex):
	...
	for wordI in range(0,inText.shape[1]):
		if freqArray[wordI] > 0.25:
			word = vocabValue[vocabIndex.index(wordI)].encode('ascii', 'ignore')
...
pageNb = 2000
classesArray = []; nodes = [];
cur.execute("SELECT id FROM wg_page ORDER BY PR DESC LIMIT "+str(pageNb))
numberClasses = 10
i = 0
...
texts = [];
for node in nodes:
	f = open('../igraph/txt/'+str(node)+'.txt', "r")
	texts.append(f.read())
	f.close()
...
while 1.0*changed > 0.01*totalCount.shape[0]:
	...
	for clas in classSet:
		...
		print clas, " | ", ','.join(results['bestKeywords']), " | ", np.where(classesArray==clas)[0].size
	f.close()
	r += 1
	changed = 0
