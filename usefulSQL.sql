...
SELECT link.*, fromTab.name AS fromName, toTab.name AS toName FROM `wg_links` AS link INNER JOIN wg_page AS fromTab ON fromTab.id=link.`from` INNER JOIN wg_page AS toTab ON toTab.id=link.`to` WHERE 1