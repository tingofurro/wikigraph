...
cur.execute("SELECT `from` FROM headerLink ORDER BY `from` DESC LIMIT 1")
res = cur.fetchall()
if len(res) > 0:
	firstId = res[0][0]
else:
...
for row in cur.fetchall():
	...
	if len(dbPush) > 5000:
		cur.execute("INSERT INTO `headerLink` (`id`, `from`, `to`) VALUES "+", ".join(dbPush)+';')
		dbPush = []
		print row[0]
