...
def isSkipSection(elem, skipping):
	...
	if elem.name =='h2':
		if elem.get_text() in badSections:
			return True
		else:
			return False
	return skipping
...
def crawlPage(idPage, name):
	fileName = 'data/'+str(idPage)+'.html'
	if not os.path.isfile(fileName):
		f = urllib.urlopen("https://en.wikipedia.org/wiki/"+name)
		html_doc = f.read()
		f.close()
		soup = BeautifulSoup(html_doc, 'html.parser')
		elemList = soup.find(id='mw-content-text').contents
