<?php
function wordScores($token) {
	if(file_exists('txt/'.$token.'Train.txt')) {
		$set = array();
		$toks = explode("||", file_get_contents('txt/'.$token.'Train.txt'));
		foreach ($toks as $i => $val) {
			$toks[$i] = '"'.$val.'"';
		}
		$r = mysql_query("SELECT * FROM wg_page WHERE name IN(".implode(", ", $toks).")");
		while($re = mysql_fetch_array($r)) {$ret = file_get_contents('data/'.$re['id'].'.txt'); array_push($set, $ret);}
