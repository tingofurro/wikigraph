...
while($re = mysql_fetch_array($r)) {
	...
	for($i = 0; $i < count($keys); $i ++) {
		...
		if(array_key_exists($keys[$i], $goodSet)) {
			$goodSet[$keys[$i]] += log($myScore);
		}
		else {
			$goodSet[$keys[$i]] = log($myScore);
		}
...
// 	for($i = 0; $i < $goTo; $i ++) {if(array_key_exists($keys[$i], $scoreWords)) {$scoreWords[$keys[$i]] -= 2*log($words[$keys[$i]]/$nbWords);}}
// }
arsort($goodSet);
// $goodSet = array_slice($goodSet, 0, 100);
print_r($goodSet);
